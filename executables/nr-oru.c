...
static void oru_scope_copy_ul_post(ORU_t *oru, int frame, int slot, int symbol)
{
  ...
  for (int aarx = 0; aarx < nant; aarx++) {
    c16_t rxdataF_fft[sym_len] __attribute__((aligned(32)));
    ...
    fftshift(rxdataF_fft, rxdataF, nbins, sym_len);
    apply_nr_rotation_symbol_fftshifted_RX(fp->symbols_per_slot,
                                           ...
                                           slot,
                                           symbol);
    memset(&ul_post[aarx * sym_len], 0, sym_len * sizeof(c16_t));
    fftshift_inverse(rxdataF, &ul_post[aarx * sym_len], nbins, sym_len);
  }
...
static void dl_symbol_process(ORU_t *oru, int frame, int slot, int symbol, c16_t **txDataF, int64_t timestamp, uint64_t abs_symbol)
{
  ...
  for (int aatx = 0; aatx < ru->nb_tx; aatx++) {
    ...
    if (do_scope && aatx < nant_scope) {
      ...
      fftshift_inverse(txDataF[aatx], &dl_pre[aatx * sym_len], nb_re, sym_len);
    }
    ...
    fftshift_inverse(txDataF[aatx], txdataF_shifted, nb_re, fp->ofdm_symbol_size);
