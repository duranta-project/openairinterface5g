...
TEST(SimClockPublisherTest, ConvertsSamplesToUnixTime)
{
  char name[64];
  snprintf(name, sizeof(name), "/oai_sim_clock_test_%d", getpid());
  ASSERT_EQ(setenv("OAI_SIM_CLOCK_SHM", name, 1), 0);

  auto publisher = SimClockPublisher::create(1000.25, 1000.0);
  ...
  ASSERT_GE(fd, 0);
  void *mapping = mmap(nullptr, sizeof(sim_clock_shared_t), PROT_READ, MAP_SHARED, fd, 0);
  ASSERT_NE(mapping, MAP_FAILED);
  const auto *clock = static_cast<const sim_clock_shared_t *>(mapping);
  EXPECT_EQ(__atomic_load_n(&clock->magic, __ATOMIC_ACQUIRE), SIM_CLOCK_MAGIC);
  EXPECT_EQ(clock->version, SIM_CLOCK_VERSION);
