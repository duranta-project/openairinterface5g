...
#define RFSIMULATOR_PARAMS_DESC {					\
  ...
  STRINGPARAM(RFSIMU_BEAM_GAINS,        "<per-beam gain/loss in dB, one value per gNB beam id>\n", simOpt, NULL,              NULL),                  \
};
// clang-format on
...
// when the gNB's antenna is on beam k, and active is driven exclusively by trx_set_beams (called
// from the real MAC/L2 beam management code, see executables/nr-ru.c:ctrl_rf()). On the UE side
// enable_beams/beam_gains are simply never configured, so get_beam_gain_db() always returns 0 and
// `active` is never touched -- there is no beam id to set for a node that has no beam.
typedef struct {
  int enable_beams;
...
// Called from trx_set_beams(), driven by the real MAC/L2 beam management code. This is the only
// place a node's beam id is ever allowed to change. One queue, shared by the tx and rx watermark
// mechanism (see the comment on beam_state_t) -- there is one beam and one schedule.
...
// space over the course of a test, without restarting the simulator. This is unrelated to which beam
// is currently active -- that's still exclusively driven by the real trx_set_beams() calls -- this
// only changes what each beam id currently means.
static int rfsimu_set_beam_gains(char *buff, int debug, telnet_printfunc_t prnt, void *arg)
{
  UNUSED(debug);
