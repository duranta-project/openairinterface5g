...
static ue_split7_device_t *make_device()
{
  ...
  priv->state = 2; // skip sync lifecycle; these tests only need write_symbols() valid

  uint32_t client_id = 0;
  assert(dev->register_client(dev, &client_id) == UE_SPLIT7_SUCCESS);
  assert(client_id == 0); // every call site below hardcodes 0 accordingly
...
static openair0_timestamp_t send_sym(ue_split7_device_t *dev, int sym, uint32_t frame = 0, uint16_t slot = 0)
{
  std::vector<c16_t> iq(FFT_SIZE, {1000, -1000});
  c16_t *bufs[1] = {iq.data()};

  mock_clear();
  dev->write_symbols(dev, 0, frame, slot, (uint8_t)sym, 1, bufs, 1);
...
static void test_ta_propagation()
{
  ...
  // next write_symbols should subtract the recorded TA
  {
    openair0_timestamp_t eff = send_sym(dev, 1);
    assert(eff == (openair0_timestamp_t)sym_start_offset(1) - (openair0_timestamp_t)last_ta_samples);
  }
  ...
  {
    openair0_timestamp_t eff = send_sym(dev, 0);
    assert(eff == 0 - (openair0_timestamp_t)last_ta_samples); // sym 0: offset = 0
  }
...
static void test_symbol_gap_continuity()
{
  ...
  std::vector<c16_t> iq(FFT_SIZE, {500, -500});
  c16_t *bufs[1] = {iq.data()};

  // Device derives symbol timing from anchor and symbol index
  std::vector<openair0_timestamp_t> recorded_ts;

  for (int sym = 0; sym < SYMS_PER_SLOT; sym++) {
    mock_clear();
    dev->write_symbols(dev, 0, 0, 0, (uint8_t)sym, 1, bufs, 1);
    assert(!mock_tx_timestamps.empty());
    recorded_ts.push_back(mock_tx_timestamps[0]);
  }

  // sym 0 air time = anchor (0) (ta=0, so no subtraction)
  assert(recorded_ts[0] == 0);
...
static void test_ta_initial_push()
{
  ...
  openair0_timestamp_t eff = send_sym(dev, 0);
  assert(eff == 0 - (openair0_timestamp_t)last_ta);
...
// TEST 8: multi-frame absolute frame timing without 1024-wrap
static void test_absolute_frame_timing_unwrap()
{
  std::cout << "[TEST 8] Multi-frame absolute frame timing without 1024-wrap\n";

  ue_split7_device_t *dev = make_device();
  dev->set_timing_advance(dev, 0);

  NR_DL_FRAME_PARMS fp = make_test_frame_parms();
  const uint64_t samples_frame = fp.samples_per_frame;

  // Frame 0:
  openair0_timestamp_t ts_f0 = send_sym(dev, 0, 0, 0);

  // Frame 1024 (past 1024-frame boundary):
  openair0_timestamp_t ts_f1024 = send_sym(dev, 0, 1024, 0);

  // Frame 2048:
  openair0_timestamp_t ts_f2048 = send_sym(dev, 0, 2048, 0);

  assert(ts_f1024 - ts_f0 == (openair0_timestamp_t)(1024 * samples_frame));
  assert(ts_f2048 - ts_f1024 == (openair0_timestamp_t)(1024 * samples_frame));

  ue_split7_device_free(dev);
  std::cout << "[PASS 8] Absolute frame timing strictly monotonic across 1024-wrap\n";
}

// TEST 9: dynamic RX and TX timing offset adjustment
static void test_timing_offset_adjustments()
{
  std::cout << "[TEST 9] Dynamic RX and TX timing offset adjustment\n";

  ue_split7_device_t *dev = make_device();
  dev->set_timing_advance(dev, 0);

  // 1. TX positive offset (+64 samples): should write 64 dummy samples, then real symbol
  dev->adjust_tx_timing_offset(dev, 64);
  mock_clear();
  std::vector<c16_t> iq(FFT_SIZE, {1000, -1000});
  c16_t *bufs[1] = {iq.data()};
  dev->write_symbols(dev, 0, 0, 0, 1, 1, bufs, 1);
  assert(mock_tx_nsamps.size() == 2);
  assert(mock_tx_nsamps[0] == 64);
  assert(mock_tx_nsamps[1] == (int)(FFT_SIZE + CP_NORMAL));
  std::cout << "  TX positive offset: dummy write of " << mock_tx_nsamps[0] << " samples, then " << mock_tx_nsamps[1] << "\n";

  // 2. TX negative offset (-32 samples): should write (FFT_SIZE + CP_NORMAL - 32) samples
  dev->adjust_tx_timing_offset(dev, -32);
  mock_clear();
  dev->write_symbols(dev, 0, 0, 0, 1, 1, bufs, 1);
  assert(mock_tx_nsamps.size() == 1);
  assert(mock_tx_nsamps[0] == (int)(FFT_SIZE + CP_NORMAL - 32));
  std::cout << "  TX negative offset: trimmed write of " << mock_tx_nsamps[0] << " samples\n";

  // 3. RX timing adjustment shifts TX in lockstep (frame boundary drift compensation)
  assert(dev->adjust_rx_timing_offset(dev, 16) == UE_SPLIT7_SUCCESS);
  mock_clear();
  dev->write_symbols(dev, 0, 0, 0, 1, 1, bufs, 1);
  assert(mock_tx_nsamps.size() == 2);
  assert(mock_tx_nsamps[0] == 16);
  assert(mock_tx_nsamps[1] == (int)(FFT_SIZE + CP_NORMAL));
  std::cout << "  RX adjustment (+16) shifted TX in lockstep: dummy write of " << mock_tx_nsamps[0] << " samples\n";

  ue_split7_device_free(dev);
  std::cout << "[PASS 9] Dynamic RX and TX timing offset adjustments verified\n";
}
...
int main(int argc, char **argv)
{
  ...
  std::cout << "---------------------------------------------------------\n";
  test_absolute_frame_timing_unwrap();
  std::cout << "---------------------------------------------------------\n";
  test_timing_offset_adjustments();
