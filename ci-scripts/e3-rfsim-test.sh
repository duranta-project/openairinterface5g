...
cleanup() {
    ...
    rm -rf "${WORK}"
    rm -f "$(dirname "${CONF}")"/.e3-ci-gnb_*.conf
}
trap cleanup EXIT
...
run_encoding() {
    ...
    # Beside the original, not in ${WORK}: the stock rfsim conf @includes
    # neighbour-config-rfsim.conf, resolved relative to the conf's own
    # directory, so a copy anywhere else dies at startup.
    local conf
    conf="$(dirname "${CONF}")/.e3-ci-gnb_${enc}.conf"
    local gnb_log="${WORK}/gnb_${enc}.log"
    local dapp_log="${WORK}/dapp_${enc}.log"

    # Derive a hermetic conf from the stock rfsim one: add the sensing keys this
    # test needs, append an E3Configuration block with the requested encoding
    # and the dApp's hardcoded tcp ports (9990 setup / 9991 publisher / 9999
    # subscriber), and point NGAP at an unreachable AMF so the run needs no core.
    awk -v enc="${enc}" '
      /amf_ip_address/ {
        print "    // Sensing, injected by ci-scripts/e3-rfsim-test.sh."
        print "    // Slot 8 of the TDD period is hard-reserved: no UE is granted UL there,"
        print "    // so the dApp sees a full symbol 0-13 sensing range for it."
        print "    sensing_target_slots     = (8);"
        print "    sensing_pusch_mcs        = 9;"
        print "    sensing_pusch_rb_size    = 40;"
        print "    sensing_pusch_rb_start   = 0;"
        print "    sensing_pusch_nrOfLayers = 1;"
        print "    sensing_pusch_beams      = (0);"
        sub(/ipv4[ \t]*=[ \t]*"[0-9.]*"/, "ipv4 = \"127.0.0.99\"")
      }
      { print }
      END {
        print ""
        print "E3Configuration : {"
        print "  link            = \"zmq\";"
        print "  transport       = \"tcp\";"
        print "  encoding        = \"" enc "\";"
        print "  setup_port      = 9990;"
        print "  publisher_port  = 9991;"
        print "  subscriber_port = 9999;"
        print "};"
      }
    ' "${CONF}" > "${conf}"
    grep -q "encoding        = \"${enc}\";" "${conf}" || { fail "${enc}: could not set encoding in the derived conf"; return; }
    grep -q "sensing_target_slots" "${conf}" || { fail "${enc}: sensing keys not injected into the derived conf"; return; }
    ...
    for shm in /dev/shm/e3_ran_buffers /dev/shm/e3_l2_sensing; do
