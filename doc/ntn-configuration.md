...
[radio/zmq/channel_emulation/README.md](../radio/zmq/channel_emulation/README.md)
...
2. **Initialize the simulation clock and start OCUDU gNB**:
  ```bash
  sudo python3 tools/ntn/init_sim_clock.py /tmp/ntn-profile.csv
  sudo env OAI_SIM_CLOCK_SHM=/oai_sim_clock \
    LD_PRELOAD="$PWD/cmake_targets/ran_build/build/libsim_clock_shim.so" \
    /path/to/ocudu_gnb ...
  ```
  ...
  UTC offset by default; set `OAI_TAI_OFFSET` if a different offset is required.
