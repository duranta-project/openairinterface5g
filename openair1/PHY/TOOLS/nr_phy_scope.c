...
static NR_DL_FRAME_PARMS *scope_frame_parms(scopeData_t *p) {
  if (p->gNB)
    return &p->gNB->frame_parms;
  if (p->ru)
    return p->ru->nr_frame_parms;
  return NULL;
}

static void gNBWaterFall (OAIgraph_t *graph, scopeData_t *p, int nb_UEs) {
  NR_DL_FRAME_PARMS *frame_parms = scope_frame_parms(p);
  if (!frame_parms)
    return;

  // FHI 7.2 / IF4+ RUs do not allocate time-domain rxdata
  if (!p->ru || !p->ru->common.rxdata || !p->ru->common.rxdata[0])
    return;
  //use 1st antenna
...
static void gNBfreqWaterFall (OAIgraph_t *graph, scopeData_t *p, int nb_UEs) {
  ...
  if (!frame_parms)
    return;
  scopeGraphData_t *rxF = p->liveData[gNBRxdataF];
  if (!rxF || !rxF->dataSize)
    return;
  // ORU mode (no gNB) feeds one symbol at a time, so size the waterfall from the live
  // buffer instead of the (much larger) full-frame gNB layout.
  int datasize = p->gNB ? frame_parms->samples_per_frame_wCP : rxF->lineSz;
  int divisions = p->gNB ? frame_parms->slots_per_frame : max(1, rxF->lineSz / frame_parms->ofdm_symbol_size);
  //use 1st antenna
...
#ifndef WEBSRVSCOPE
// --- Standalone O-RU DL/UL x pre/post-channel I/Q constellations ---------------
// One window, four fixed frequency-domain plots forming the DL/UL x pre/post-channel matrix.
// Each scope buffer holds one OFDM symbol for ALL antenna ports (colSz = number of ports,
// antenna-major layout), populated in executables/nr-oru.c:
...
// Non-zero REs from all ports are packed into a single series (nrscope's point symbol is
// always yellow, so per-port overlays would not read as different colours anyway).
//
// The plot count is fixed at four regardless of antenna count. All plots use a fixed
// symmetric axis range (±ORU_IQ_AXIS) so the constellations never rescale between frames.
#define ORU_IQ_AXIS 750.0f
#define ORU_IQ_MAX_ANT 4

// Plot non-zero REs on the active carrier grid only (drop guard-band / empty PRBs).
static void oruConstellation(OAIgraph_t *graph, scopeData_t *p, enum scopeDataType type)
{
  // Only refresh when the RT path copied new samples for this tap. Otherwise return without
  // touching the plot: the xyplot keeps its last dataset, so the previous constellation stays
  // on screen instead of being repainted (empty/stale buffers would otherwise flicker to black).
  static uint64_t lastCnt[EXTRA_SCOPE_TYPES];
  uint64_t cnt = p->liveDataCnt[type];
  if (cnt == lastCnt[type])
    return;
  lastCnt[type] = cnt;

  scopeGraphData_t *val = p->liveData[type];
  if (!val || !val->dataSize || val->lineSz <= 0)
    return;
  // dataSize is the byte size of the data area (excludes the header). lineSz is the number of
  // samples per antenna port (one OFDM symbol); colSz is the number of ports stored back to back.
  const int cap = val->dataSize / (int)sizeof(scopeSample_t);
  const int per_ant = val->lineSz;
  if (cap <= 0 || per_ant <= 0)
    return;
  int nant = val->colSz > 0 ? val->colSz : 1;
  if (nant > ORU_IQ_MAX_ANT)
    nant = ORU_IQ_MAX_ANT;
  if (nant * per_ant > cap) // never read past the live buffer
    nant = cap / per_ant;
  if (nant <= 0)
    return;

  // Active carrier grid (N_RB*12), wrapping at first_carrier_offset.
  NR_DL_FRAME_PARMS *fp = scope_frame_parms(p);
  const bool ul = (type == oruULpreFreq || type == oruULpostFreq);
  int nb_re = per_ant;
  int fco = 0;
  int fft = per_ant;
  if (fp) {
    const int nb_rb = ul ? fp->N_RB_UL : fp->N_RB_DL;
    if (nb_rb > 0 && fp->ofdm_symbol_size > 0 && fp->ofdm_symbol_size <= per_ant) {
      nb_re = nb_rb * 12;
      fft = fp->ofdm_symbol_size;
      if (nb_re > fft)
        nb_re = fft;
      // Per-link first_carrier_offset (fp->first_carrier_offset is DL-only).
      fco = fft - nb_re / 2;
    }
  }

  scopeSample_t *s = (scopeSample_t *)(val + 1);

  // Fixed axes: same symmetric ±ORU_IQ_AXIS bounds on every plot so nothing rescales/jumps.
  fl_set_xyplot_xbounds(graph->graph, -ORU_IQ_AXIS, ORU_IQ_AXIS);
  fl_set_xyplot_ybounds(graph->graph, -ORU_IQ_AXIS, ORU_IQ_AXIS);

  // Fixed length so copyData()/getbuff never realloc under the reader; pad unused slots.
  const int plot_len = nant * nb_re;
  float *I = NULL, *Q = NULL;
  oai_xygraph_getbuff(graph, &I, &Q, plot_len, 0);
  if (!I || !Q)
    return;

  int w = 0;
  for (int a = 0; a < nant; a++) {
    const scopeSample_t *sa = s + (size_t)a * per_ant;
    int re = fco;
    for (int k = 0; k < nb_re; k++) {
      if (sa[re].r != 0 || sa[re].i != 0) {
        I[w] = sa[re].r;
        Q[w] = sa[re].i;
        w++;
      }
  ...
  if (w == 0)
    return;
  for (int k = w; k < plot_len; k++) {
    I[k] = I[0];
    Q[k] = Q[0];
  }
  oai_xygraph(graph, I, Q, plot_len, 0, 0);
}

static void oruDLpreIQ (OAIgraph_t *g, scopeData_t *p, int u) { (void)u; oruConstellation(g, p, oruDLpreFreq); }
static void oruDLpostIQ(OAIgraph_t *g, scopeData_t *p, int u) { (void)u; oruConstellation(g, p, oruDLpostFreq); }
static void oruULpreIQ (OAIgraph_t *g, scopeData_t *p, int u) { (void)u; oruConstellation(g, p, oruULpreFreq); }
static void oruULpostIQ(OAIgraph_t *g, scopeData_t *p, int u) { (void)u; oruConstellation(g, p, oruULpostFreq); }

static OAI_phy_scope_t *create_phy_scope_oru_iq(void)
{
  FL_OBJECT *obj;
  OAI_phy_scope_t *fdui = calloc_or_fail((sizeof *fdui), 1);
  fdui->phy_scope = fl_bgn_form(FL_NO_BOX, 820, 820);
  fl_set_form_dblbuffer(fdui->phy_scope, 1);
  obj = fl_add_box(FL_BORDER_BOX, 0, 0, 820, 820, "");
  fl_set_object_color(obj, FL_BLACK, FL_WHITE);

  // DL/UL x pre/post-channel matrix: DL left, UL right; pre-channel top, post-channel bottom.
  // Point colour is yellow (nrscope drawsymbol); all antenna ports share one series per plot.
  fdui->graph[0] = gNBcommonGraph(oruDLpreIQ,  FL_POINTS_XYPLOT,  10,  10, 380, 380,
                                  "DL pre-channel I/Q", FL_YELLOW);
  fdui->graph[1] = gNBcommonGraph(oruULpreIQ,  FL_POINTS_XYPLOT, 420,  10, 380, 380,
                                  "UL pre-channel I/Q", FL_YELLOW);
  fdui->graph[2] = gNBcommonGraph(oruDLpostIQ, FL_POINTS_XYPLOT,  10, 420, 380, 380,
                                  "DL post-channel I/Q", FL_YELLOW);
  fdui->graph[3] = gNBcommonGraph(oruULpostIQ, FL_POINTS_XYPLOT, 420, 420, 380, 380,
                                  "UL post-channel I/Q", FL_YELLOW);
  fdui->graph[4].graph = NULL;
  // Fixed symmetric axes (±ORU_IQ_AXIS) for all constellations; oruConstellation()
  // re-asserts the same bounds and never rescales them.
  for (int i = 0; i < 4; i++) {
    fl_set_xyplot_xbounds(fdui->graph[i].graph, -ORU_IQ_AXIS, ORU_IQ_AXIS);
    fl_set_xyplot_ybounds(fdui->graph[i].graph, -ORU_IQ_AXIS, ORU_IQ_AXIS);
  }
  fl_end_form();
  if (fdui->phy_scope)
    fdui->phy_scope->fdui = fdui;
  fl_show_form(fdui->phy_scope, FL_PLACE_HOTSPOT, FL_FULLBORDER, "O-RU I/Q constellations");
  return fdui;
}
...
static void *scope_thread_gNB(void *arg) {
  ...
  // Standalone O-RU (no gNB context) only gets the dedicated pre/post I/Q constellation window;
  // the generic "NR UL SCOPE gNB" form is gNB-only (its PUSCH/PUCCH/throughput plots need a real
  // gNB and would show up as a second, mostly-empty window for an O-RU).
  bool is_oru = (p->ru && !p->gNB);
  OAI_phy_scope_t *form_gnb = is_oru ? NULL : create_phy_scope_gnb();
  OAI_phy_scope_t *form_iq = is_oru ? create_phy_scope_oru_iq() : NULL;
  ...
  while (!oai_exit) {
    ...
    if (form_iq) {
      fl_freeze_form(form_iq->phy_scope);
      phy_scope_gNB(form_iq, p, nb_ue);
      fl_unfreeze_form(form_iq->phy_scope);
      fl_redraw_form(form_iq->phy_scope);
    }
    usleep(99*1000);
  }
...
{
  AssertFatal(p->gNB || p->ru, "nrscope needs a gNB or RU pointer\n");
  void **scopeData = p->gNB ? &p->gNB->scopeData : &p->ru->scopeData;
  AssertFatal(*scopeData = calloc_or_fail(sizeof(scopeData_t), 1), "");
...
#ifndef WEBSRVSCOPE
  threadCreate(&forms_thread, scope_thread_gNB, *scopeData, "scope", -1, OAI_PRIORITY_RT_LOW);
...
void nrscope_autoinit(void *dataptr) {
  AssertFatal(dataptr != NULL, "Scope init called without a context\n");

  // The 5G UE passes a PHY_VARS_NR_UE*; the gNB and the standalone O-RU (nr-oru)
  // both pass a scopeParms_t*. The O-RU sets neither the GNB nor the 5GUE softmodem
  // bit, so route by "is this the UE?" and let gNBinitScope validate the gNB/RU pointer.
  if (IS_SOFTMODEM_5GUE)
