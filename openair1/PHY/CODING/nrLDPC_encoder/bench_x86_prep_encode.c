 ...
 * too, and no permutes.
 *
 * The permutex stock encoder needs AVX512VBMI, so on an AVX512 part without it
 * (Skylake-SP, Cascade Lake) that row is omitted and the stock baseline is the
 * AVX2 path alone. The factored encoders need only AVX512F.
 *
...
#error "build with -march=native on an AVX512 part"
#endif
