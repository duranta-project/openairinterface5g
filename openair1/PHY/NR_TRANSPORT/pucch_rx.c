...
#endif	       
      	      // unscrambling here. Only datacnt+1 lanes hold valid symbols: the final batch
	      // is partial when the number of data symbols is not a multiple of 4 (e.g. 4-symbol
	      // PUCCH 3 has 3 data symbols). Interleaved column c holds data symbol s3-datacnt+c.
	      for (int c = 0; c <= datacnt; c++) {
                 const int tsym = s3 - datacnt + c;
#ifdef DEBUG_NR_PUCCH_RX
	         printf("Unscrambling symbol %d (s3 %d datacnt %d col %d) offset %d\n", tsym, s3, datacnt, c, tsym*nb_re_data/4);
#endif
                 simde__m128i *c_ptr = (simde__m128i *)scramb_data + (tsym*nb_re_data/4);
                 simde__m128i *br_ptr = (simde__m128i *)r_ext[aa][tsym];
                 simde__m128i *bi_ptr = (simde__m128i *)r_ext2[aa][tsym];
		 for (int i=0; i < nb_re_data/4 ; i++) {
		     // note: conjugate here completes the idft above
                     simde__m128i tmp = simde_mm_srai_epi16(oai_mm_conj(interleaved_out[aa][(c*nb_re_data/4)+i]), scaling);
