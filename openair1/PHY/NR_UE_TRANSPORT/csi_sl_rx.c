...
#include "openair1/PHY/nr_phy_common/inc/nr_phy_common_csi_rs.h"
...
void nr_det_A_MF_2x2(c16_t *a_mf_00,
                     c16_t *a_mf_01,
                     c16_t *a_mf_10,
                     c16_t *a_mf_11,
                     int32_t *det_fin,
...
void nr_squared_matrix_element(c16_t *a,
                               int32_t *a_sq,
                               const unsigned short nb_rb) {
...
int nr_csi_rs_ri_estimation(const PHY_VARS_NR_UE *ue,
  ...
  */
  ...
  c16_t csi_rs_estimated_conjch_ch[frame_parms->nb_antennas_rx][N_ports][frame_parms->nb_antennas_rx][N_ports][frame_parms->ofdm_symbol_size + FILTER_MARGIN] __attribute__((aligned(32)));
  c16_t csi_rs_estimated_A_MF[N_ports][N_ports][frame_parms->ofdm_symbol_size + FILTER_MARGIN] __attribute__((aligned(32)));
        ...
        for (int ant_rx_ch = 0; ant_rx_ch < frame_parms->nb_antennas_rx; ant_rx_ch++) {
          for(uint16_t port_tx_ch = 0; port_tx_ch < N_ports; port_tx_ch++) {
            ...
            if(ant_rx_conjch == ant_rx_ch) {
              nr_a_sum_b(
                  &csi_rs_estimated_A_MF[port_tx_conjch][port_tx_ch][k_offset], &csi_rs_estimated_conjch_ch[ant_rx_conjch][port_tx_conjch][ant_rx_ch][port_tx_ch][k_offset], 1);
            }
          }
        }
      }
                                ...
                                fapi_nr_dl_config_csirs_pdu_rel15_t *csirs_config_pdu)
{
  ...
  csi_mapping_parms_t mapping_parms = get_csi_mapping_parms(csirs_config_pdu->row,
                                                             ...
                                                             csirs_config_pdu->symb_l0,
                                                             csirs_config_pdu->symb_l1);
  N_cdm_groups = mapping_parms.size;
  CDM_group_size = get_cdm_group_size(csirs_config_pdu->cdm_type);
  k_prime = mapping_parms.kprime;
  l_prime = mapping_parms.lprime;
  N_ports = mapping_parms.ports;
  for (int _i = 0; _i < 16; _i++) {
    j_cdm[_i] = mapping_parms.j[_i];
    k_overline[_i] = mapping_parms.koverline[_i];
    l_overline[_i] = mapping_parms.loverline[_i];
  }
...
//        get_nrUE_params()->snr = sl_sinr_dB;
  ...
  fapi_nr_l1_measurements_t l1_measurements = {
    ...
    //.i2 = *i2,
    .cqi = cqi,
    .radiolink_monitoring = RLM_no_monitoring, // TODO to be activated in case of RLM based on CSI-RS
  };
