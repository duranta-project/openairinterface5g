...
void nr_dlsim_preprocessor(gNB_MAC_INST *nr_mac, post_process_pdsch_t *pp_pdsch)
{
  ...
  NR_UE_info_t *UE_info = find_nr_UE(&nr_mac->UE_info, g_rnti);
  DevAssert(UE_info && UE_info->rnti == g_rnti);
...
int main(int argc, char **argv)
{
  ...
  secondaryCellGroup->spCellConfig->reconfigurationWithSync = get_reconfiguration_with_sync(g_rnti, uid, scc, frame);
  ...
  nr_mac_add_test_ue(RC.nrmac[0], g_rnti, secondaryCellGroup);
  // reset preprocessor to the one of DLSIM after it has been set during
  // nr_mac_config_scc()
  gNB_mac->pre_processor_dl = nr_dlsim_preprocessor;
  phy_init_nr_gNB(gNB);
  N_RB_DL = gNB->frame_parms.N_RB_DL;
  NR_UE_info_t *UE_info = find_nr_UE(&RC.nrmac[0]->UE_info, g_rnti);
