...
int is_xnap_enabled(void)
{
  ...
  config_get(config_get_if(), XnParams, nb_xn_params, aprefix);
  char **gnb_xn_ip_strptr = gpd(XnParams, nb_xn_params, GNB_CONFIG_STRING_GNB_IPV4_ADDRESS_FOR_XNC)->strptr;
  uint32_t gnb_port_for_xnc = *gpd(XnParams, nb_xn_params, GNB_CONFIG_STRING_GNB_PORT_FOR_XNC)->uptr;

  int xn_enabled = (gnb_xn_ip_strptr != NULL) && (*gnb_xn_ip_strptr != NULL) && (gnb_port_for_xnc != 0);
  LOG_I(XNAP, "Xn interface %s (gNB Xn-C address %s, port %u)\n",
        xn_enabled ? "enabled" : "disabled", xn_enabled ? *gnb_xn_ip_strptr : "not configured", gnb_port_for_xnc);
