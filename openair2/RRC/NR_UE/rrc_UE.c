  ...
  if (radioBearerConfig->srb_ToAddModList != NULL) {
    for (int cnt = 0; cnt < radioBearerConfig->srb_ToAddModList->list.count; cnt++) {
	  ...
	  if (full_config && rrc->Srb[srb->srb_Identity] != RB_NOT_PRESENT) {
		...
		// TS 38.331 ?5.3.5.11 full config: apply default RLC config (9.2.1.1/9.2.1.2) for SRB1/SRB2  
		nr_rlc_reconfigure_entity(rrc->ue_id, srb->srb_Identity, NULL); // lc_id
	  }
	...
	if (masterCellGroup) {
		...
		if(spCellConfig) {
		  ...
		  if (reconfigurationWithSync) {
			...
			mac_msg->hfn = rrc->current_hfn;
			mac_msg->frame = rrc->current_frame;
			//Will also apply default phy configuration with nr_ue_mac_default_configs at mac thread
			mac_msg->full_config = true;
			nr_rrc_send_msg_to_mac(rrc, &rrc_msg);
    ...
    if (rec_1560) {
      ...
      if (rec_1560->radioBearerConfig2) {
        ...
        if ((dec_rval.code != RC_OK) && (dec_rval.consumed == 0)) {
          ...
          nr_rrc_ue_process_RadioBearerConfig(rrc, RadioBearerConfig, rec_1530->fullConfig);
      }
    }
  }
  ...
  switch (reconfiguration->criticalExtensions.present) {
    case NR_RRCReconfiguration__criticalExtensions_PR_rrcReconfiguration: {
      ...
      if (ie->radioBearerConfig) {
        ...
        nr_rrc_ue_process_RadioBearerConfig(rrc, ie->radioBearerConfig, false);
        if (LOG_DEBUGFLAG(DEBUG_ASN1))
