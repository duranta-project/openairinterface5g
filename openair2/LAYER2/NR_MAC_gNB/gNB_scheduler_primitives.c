...
static void set_periodic_info(nr_cell_sched_t *cell, NR_UE_info_t *UE, nr_periodic_channel_t channel, int offset)
{
  ...
  switch (channel) {
    case SRS:
      ...
      bool reset = false;
      if (UE->UE_sched_ctrl.sched_srs.periodic_sched.periodic_offset != -1) {
        if (UE->UE_sched_ctrl.sched_srs.periodic_sched.periodic_offset == idx) {
          for (int i = 0; i < cell->periodic_srs_config.max_ue_per_slot; i++) {
            if (*get_periodic_ue(&cell->periodic_srs_config, idx, i) == UE)
              return; // info already set for this UE
          }
          reset = true;
        } else
          reset = true;
      ...
      if (reset)
        reset_periodic_info(cell, UE, SRS); // need to re-configure
      for (int i = 0; i < cell->periodic_srs_config.max_ue_per_slot; i++) {
