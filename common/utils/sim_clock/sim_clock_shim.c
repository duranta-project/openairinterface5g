...
static sim_clock_shared_t *shared_clock;
static unsigned int map_attempts;

static sim_clock_shared_t *get_shared_clock(void)
{
  sim_clock_shared_t *clock = __atomic_load_n(&shared_clock, __ATOMIC_ACQUIRE);
  if (clock != NULL)
    return clock;
  ...
  const char *name = configured_name != NULL ? configured_name : SIM_CLOCK_DEFAULT_SHM;
  const int fd = shm_open(name, O_RDONLY, 0);
  if (fd < 0)
    return NULL;
  void *mapping = mmap(NULL, sizeof(sim_clock_shared_t), PROT_READ, MAP_SHARED, fd, 0);
  close(fd);
  if (mapping == MAP_FAILED)
    return NULL;

  sim_clock_shared_t *expected = NULL;
  if (!__atomic_compare_exchange_n(&shared_clock, &expected, mapping, false, __ATOMIC_RELEASE, __ATOMIC_ACQUIRE)) {
    munmap(mapping, sizeof(sim_clock_shared_t));
  }
...
{
  sim_clock_shared_t *clock = get_shared_clock();
  if (clock == NULL || __atomic_load_n(&clock->magic, __ATOMIC_ACQUIRE) != SIM_CLOCK_MAGIC || clock->version != SIM_CLOCK_VERSION)
    return -1;

  uint64_t time_ns = __atomic_load_n(&clock->unix_time_ns, __ATOMIC_ACQUIRE);
  if (clock_id == CLOCK_TAI) {
    const char *configured_offset = getenv("OAI_TAI_OFFSET");
